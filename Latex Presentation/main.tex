...
\textbf{Better classical post-processing.} Brute-force trial division is the simplest way to consume the annealer's guess $(P_{\text{pred}}, Q_{\text{pred}})$, but it makes almost no use of the prediction beyond using it as a starting point. The annealer hands us a rich piece of information — an approximate factor of $N$ — and existing classical factorization techniques may be able to exploit this kind of partial information far more effectively than a linear scan. Exploring such methods is a natural next step.

\medskip
\textbf{The real bottleneck: annealer solution quality.} From the brute-force timing model
\[
\text{Time}(\text{s}) \;=\; \frac{3.32 \times 10^{-9}}{\text{threads}} \,\times\, \text{Gap}(\%) \,\times\, 2^n,
\]
two levers control the total cost:
\begin{itemize}
  \item \textbf{Threads:} linear scaling — pure compute throw. Useful but bounded by hardware.
  \item \textbf{Gap (\%):} a multiplicative factor in front of $2^n$. Since $2^n$ grows exponentially with bit-length, \emph{shrinking the gap is the only lever that can keep up} with larger $N$.
\end{itemize}

\medskip
\textbf{Therefore the highest-impact direction is improving the annealer:} tighter QUBO encodings, larger neighborhoods, better cooling schedules, parallel-tempering or population annealing on top of the GPU SA — anything that drives $P_{\text{pred}}$ closer to $P$ and chips away at $\text{Gap}(\%)$.
