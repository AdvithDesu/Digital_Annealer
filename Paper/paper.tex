...
The brute-force search scales as $\mathrm{Gap}(\%)\cdot 2^n$, so trial division around $P_{\mathrm{centre}}$ is not possible at cryptographic sizes. Future work could explore integrating other classical algorithms that can better take advantage of an approximate solution of P (or Q). One such example is the Coppersmith's method~\cite{Coppersmith1997}, which uses lattice (LLL) reduction to recover a factor $P$ of $N = PQ$ in polynomial time whenever an approximation lies within $N^{1/4}$ of the true value. Improving the solution quality of the annealer would allow us to explore such post-processing algorithms and push the end-to-end factoring to larger bit widths.
