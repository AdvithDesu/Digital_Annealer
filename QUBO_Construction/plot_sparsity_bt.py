...
d = sys.argv[1] if len(sys.argv) > 1 else "/tmp/bt_out"
N = "159197"
...
ax[0].set_title(f"C++ J sparsity (backtracking) N={N}, n={n}")
im = ax[1].imshow(M, cmap="seismic", vmin=-np.max(np.abs(M)), vmax=np.max(np.abs(M)))
ax[1].set_title(f"C++ J magnitudes (backtracking) N={N}")
plt.colorbar(im, ax=ax[1])
plt.tight_layout()
out = f"{d}/sparsity_bt_{N}.png"
